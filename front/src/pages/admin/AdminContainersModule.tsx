...
export default function AdminContainersModule() {
  ...
  const chartData = useMemo(() => [
    ...
    { label: 'Occupied', value: stats.occupied, color: 'var(--mantine-color-yellow-6)' },
  ...
  return (
    <MantineContainer px="md" size="xl">
        ...
        >
          {filteredData.map((c) => (
            ...
            >
              ...
              <Table.Td ta="center">
                <Pill 
                  style={{ backgroundColor: c.status === 'ready' ? '#45a575' : c.status === 'occupied' ? 'var(--mantine-color-yellow-6)' : 'var(--mantine-color-red-6)' }}
                  c="white"
                >
      ...
      {/* create */}
      <Modal opened={openedCreate} onClose={closeCreate} title="Deploy New Container" centered>
        <form onSubmit={handleCreateSubmit}>
          <Stack>
            <TextInput label="City Name" name="city" required placeholder="e.g. Lyon" />
            <TextInput label="Postal Code" name="zip" required placeholder="e.g. 69000" />
      ...
      {/* delete */}
      <Modal opened={openedDelete} onClose={closeDelete} title="Confirm Archiving" centered>
        <Text size="sm" mb="lg">
          Are you sure you want to remove container <strong>#{selectedContainer?.id}</strong> ({selectedContainer?.city_name})? 
