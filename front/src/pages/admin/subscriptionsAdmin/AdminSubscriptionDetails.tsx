...
export default function AdminSubscriptionDetails() {
  ...
  const [openedRevoke, { open: openRevoke, close: closeRevoke }] = useDisclosure(false);
  const [cancelReason, setCancelReason] = useState("");

  const { data: sub, isLoading, isError } = useGetSubscriptionByID(subscriptionId);
  const revokeMutation = useRevokeSubscription();

  const handleRevoke = () => {
    revokeMutation.mutate(
      { id: subscriptionId, cancel_reason: cancelReason },
      { onSuccess: () => { closeRevoke(); navigate(PATHS.ADMIN.SUBSCRIPTIONS.ALL); } },
    );
  };

  if (isLoading) return <FullScreenLoader />;
  if (!isValidId || isError) return <Navigate to={PATHS.ADMIN.SUBSCRIPTIONS.ALL} replace />;
    ...
    <Container px="md" size="xl">
      ...
      <Container px="md" size="sm" mt="xl">
        <Stack justify="center" align="center" mb="xl">
          <ThemeIcon size={80} radius="xl" color={sub?.is_active ? "violet" : "gray"}>
            <IconCrown size={45} />
          </ThemeIcon>
          <Avatar src={sub?.avatar} size="xl" radius="xl" />
          <Title order={2}>{sub?.username}</Title>
          <Badge color={sub?.is_active ? "green" : "red"} size="lg">
            {sub?.is_active ? "Active" : "Canceled"}
          </Badge>
        </Stack>

        <Title order={3} ta="left" mt="xl">Subscription Information</Title>
      ...
      <Modal opened={openedRevoke} onClose={closeRevoke} title="Revoke Subscription" centered>
        <Textarea
          label="Cancel Reason"
        ...
        <Group mt="xl" justify="flex-end">
          <Button variant="grey" onClick={closeRevoke}>Cancel</Button>
          <Button
            variant="delete"
