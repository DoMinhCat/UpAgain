...
func GetValidationRoutes(mux *http.ServeMux) {
	mux.Handle("GET /admin/validations/{$}", middleware.AuthMiddleware([]string{"admin"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetPendingValidations))))
