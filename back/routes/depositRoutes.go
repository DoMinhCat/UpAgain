...
func GetDepositRoutes(mux *http.ServeMux) {
	...
	mux.Handle("PATCH /deposits/{deposit_id}/transfer/{container_id}/{$}", middleware.AuthMiddleware([]string{"admin"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.TransferContainerByDepositId))))
}
