...
func GetEventRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /events/mine/{$}", middleware.AuthMiddleware([]string{"user", "pro", "employee"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetMyEventsByAccountId))))
