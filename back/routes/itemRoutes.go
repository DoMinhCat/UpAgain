...
func GetItemRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /items/{item_id}/transactions/{$}", middleware.AuthMiddleware([]string{"admin", "user"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetItemTransactions))))
