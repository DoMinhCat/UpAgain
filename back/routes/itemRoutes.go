...
func GetItemRoutes(mux *http.ServeMux) {
	...
	mux.Handle("POST /items/{item_id}/cancel", middleware.AuthMiddleware([]string{"pro", "admin"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.CancelItemReservation))))
	mux.Handle("POST /items/{item_id}/confirm", middleware.AuthMiddleware([]string{"user"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.ConfirmListingRetrieval))))
	...
	mux.Handle("GET /items/{item_id}/transactions/latest", middleware.AuthMiddleware([]string{"user", "pro"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetLatestTransaction))))
