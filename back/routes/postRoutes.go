...
func GetPostRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /posts/accounts/{account_id}/me{$}", middleware.AuthMiddleware([]string{"pro", "employee"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetPostsByAccountId))))
	mux.Handle("GET /posts/accounts/{account_id}/saved{$}", middleware.AuthMiddleware([]string{"pro", "employee"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetSavedPosts))))
