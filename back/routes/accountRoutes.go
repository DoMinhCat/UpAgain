...
func GetAccountRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /accounts/{id_account}/{$}", middleware.AuthMiddleware([]string{"admin"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetAccountDetails))))
	...
	mux.Handle("GET /accounts/{id_account}/stats/{$}", middleware.AuthMiddleware([]string{"admin"}, middleware.UpdateLastActive(http.HandlerFunc(controllers.GetAccountStats))))
