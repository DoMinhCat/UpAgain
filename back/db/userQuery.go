...
func UpdateOnboardByIdAccount(id_account int, role string) error {
	var err error
	if role == "pro" {
		_, err = utils.Conn.Exec("UPDATE pros SET completed_onboard=true WHERE id_account=$1", id_account)
	} else {
