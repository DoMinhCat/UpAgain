...
func GetAssignedEventsByEmployeeId(id_employee int) ([]models.Event, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&event.Id, &event.CreatedAt, &event.Title, &event.Description, &event.StartAt, &event.EndAt, &event.Price, &event.Category, &event.Capacity, &event.Status, &event.City, &event.Street, &event.PostalCode, &event.LocationDetail, &event.EmployeeName, &event.EmployeeAvatar, &event.EmployeeId, &event.Registered)
		if err != nil {
