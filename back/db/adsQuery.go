...
func GetTotalAdsSpendingsById(id int) (float64, error) {
	total := 0.0
	...
	if err != nil {
		return 0.0, fmt.Errorf("GetTotalAdsSpendingsById() failed: %v", err.Error())
	}
	...
	for rows.Next() {
		...
		total += (ads_price / 30.0) * float64(end_date.Sub(start_date).Hours()/24)
	}
	return total, nil
}
